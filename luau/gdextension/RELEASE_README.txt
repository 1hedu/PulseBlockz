...
Three apps, one engine. Run any .exe. They share the .dll in this folder, so keep them together.

  PulseBlockz Studio.exe      Edit a place. Opens .rbxl and .rbxlx files from Roblox.
  PulseBlockz.exe             The Player. Opens published places and joins servers.
  PulseBlockz Publisher.exe   Puts a place on chain.
...
Studio fetches them signed in as you. Put your .ROBLOSECURITY cookie in
  ...
  %APPDATA%\Godot\app_userdata\PulseBlockz Studio\roblox_cookie.txt
...
Your wallet
-----------
Each app keeps its key in its own data folder under %APPDATA%\Godot\app_userdata\, never in
