# Client-side curation: signed block/allow lists, merged, hiding assets, creators and servers.
# The chain has no delist switch; hiding is this client refusing to render, the token stays in
# the wallet. Autoload as `Curation`. See ../../../curation/SPEC.md, curation/src/curation.js.
extends Node
...
var verifier: Callable                    # EIP-712 over keccak-256, secp256k1 recovery
...
# ---- lists ------------------------------------------------------------------------
## Stores a list; per maintainer the highest `updated` wins. False if rejected.
...
# ---- reporting --------------------------------------------------------------------
## POSTs a report to every enabled list that publishes a `report` endpoint.
