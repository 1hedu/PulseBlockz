...
// These run on the WORKER thread. They never touch the node or the scene tree:
// they validate arguments, emit a Command event, and return. The node applies
// the command on the main thread in _drain_events() as a signal.
...
// Destroying the worker joins its thread first, so no event can be produced past
// this point. A _drain_events call_deferred that is still queued is dropped by the
// engine because the object is gone.
...
void LuauScriptHost::ensure_worker() {
    ...
    if (opts_.threaded) {
        // Runs on the worker thread when the event queue goes empty -> non-empty.
        // call_deferred goes through the engine's MessageQueue, which is the
        // documented thread-safe way to get back onto the main thread.
...
// A script's parts die with it. The unload is queued behind whatever the worker
// is still running for this script, so commands it emits from here on are dropped
// in _drain_events (the handle is no longer live) rather than orphaning parts.
void LuauScriptHost::unload_script(int handle) {
    if (!worker_) return;
    live_.erase(handle);
    auto it = owned_.find(handle);
    if (it != owned_.end()) {
        std::vector<int64_t> ids = std::move(it->second);
        owned_.erase(it);
        for (int64_t id : ids) emit_signal("part_destroy_requested", id);
    }
...
void LuauScriptHost::_drain_events() {
    ...
    for (const HostEvent& e : worker_->drain()) {
        ...
        case HostEvent::Command:
            if (!live_.count(e.script)) break;   // script unloaded meanwhile; its parts are already gone
            if (e.text == "part.create") {
                owned_[e.script].push_back(e.id);
                emit_signal("part_create_requested", e.id, Vector3(e.x, e.y, e.z));
            } else if (e.text == "part.destroy") {
                auto& ids = owned_[e.script];
                for (size_t i = 0; i < ids.size(); i++)
                    if (ids[i] == e.id) { ids[i] = ids.back(); ids.pop_back(); break; }
                emit_signal("part_destroy_requested", e.id);
            } else if (e.text == "part.move") {
                emit_signal("part_move_requested", e.id, Vector3(e.x, e.y, e.z));
            }
            break;
        }
    }
